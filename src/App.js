...
import { TrendingUp, Plus, Users, Settings, Shuffle, Shield } from 'lucide-react'; // <-- ADDED Shield icon
...
import GoalkeeperStats from './components/GoalkeeperStats'; // <-- ADDED GoalkeeperStats
...
        <Header currentUser={currentUser} onLogout={handleLogout} error={error} showThemeToggle={true} />
...
              </button>
            )}
            <button // <-- ADDED Goalkeepers Tab
              onClick={() => setActiveTab('goalkeeper')}
              className={`flex-1 min-w-max px-4 sm:px-6 py-3 sm:py-4 font-semibold flex items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base ${
                activeTab === 'goalkeeper' ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'
              }`}
            >
              <Shield size={18} />
              Goalkeepers
...
        {activeTab === 'goalkeeper' && ( // <-- ADDED GoalkeeperStats Content
